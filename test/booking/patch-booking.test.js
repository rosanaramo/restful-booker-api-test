...
} = require("../../helpers/datesHelper.js");
...
describe("Patch/booking", () => {
  ...
  describe("Partial updating", () => {
    it("Should update only the first name of a booking", async () => {
      ...
      expect(persistedBooking.body.firstname).to.be.equal(
        bookingBody.firstname,
      );
    });
    ...
    it("Should update only the totalprice of a booking", async () => {
      ...
      const persistedBooking = await getBooking(bookingId);
      expect(persistedBooking.body.totalprice).to.be.equal(
        bookingBody.totalprice,
      );
    });
    ...
    it("Should update only the depositpaid of a booking", async () => {
      ...
      const persistedBooking = await getBooking(bookingId);
      expect(persistedBooking.body.depositpaid).to.be.equal(
        bookingBody.depositpaid,
      );
    });
    ...
    it("Should update only booking dates", async () => {
      ...
      const persistedBooking = await getBooking(bookingId);
      expect(persistedBooking.body.bookingdates).to.be.deep.equal(
        response.body.bookingdates,
      );
    });
    ...
    it("Should update only the additionalneeds  of a booking", async () => {
      ...
      const persistedBooking = await getBooking(bookingId);
      expect(persistedBooking.body.additionalneeds).to.be.equal(
    ...
    it("Should update only the provided fields", async () => {
      ...
      expect(persistedBooking.body.depositpaid).to.be.equal(
        booking.body.depositpaid,
      );
      expect(persistedBooking.body.bookingdates).to.be.deep.equal(
        booking.body.bookingdates,
      );
    });
  });
  ...
  describe("Business Rules", () => {
    it("Should allow update a booking with total price equals 0", async () => {
      ...
      const persistedBooking = await getBooking(bookingId);
      expect(persistedBooking.body.totalprice).to.be.deep.equal(
        bookingBody.totalprice,
      );
    });
